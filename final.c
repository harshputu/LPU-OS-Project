...
{
	int i,n;
	int burstTime[20],temp,j,arrivalTime[20],waitTime[20],turnAroundTime[20],ta=0,csum=1,completionTime[20],real=1,turnAroundTime1[20],completionTime1[20];
	...
	for(j=1;j<n;j++)
	{
		for(i=1;i<n;i++)
		{
			if (burstTime[i]>=arrivalTime[i]  && burstTime[i]>burstTime[j] )
			{
				temp=process[j];
				process[j]=process[i];
				process[i]=temp;
	...
	{	
		...
		completionTime1[i]=turnAroundTime1[i]+arrivalTime[i];
	...
	printf("---------------------------------------------------------------------------------------");
	printf("\n\n\n\n RESULT:-");
	printf("\nProcess\t Burst\t Arrival\t Waiting\t Turn-around\t Real Time\t Completion(2 Unit) " );
	for(i=0;i<n;i++)
	{
		printf("\n p%d\t %d\t %d\t\t %d\t\t\t%d\t\t%d\t\t%d",process[i],burstTime[i],arrivalTime[i],waitTime[i],turnAroundTime[i],completionTime1[i],completionTime[i]);
	}
